...
function triggerGoal(scorerText, concedingTeam) {
...
// Solid Box Collision Resolver (Prevents Teleporting/Respawning)
function resolveBoxCollision(player, box) {
    // Determine closest point on the penalty box boundary
    ...
    // If player circle overlaps or is inside box, push them smoothly outside
    if (distance < player.radius) {
        // If player managed to end up entirely inside, calculate push distances to all 4 edges
        if (distance === 0) {
            ...
            // Push out along collision vector smoothly
...
function update() {
    ...
    if (activeRed) {
        ...
        if (activeRed.isGk) {
            ...
            // Prevent entering Blue box smoothly from ALL sides if Blue GK has ball
            if (blueGkHasBall) {
    ...
    if (activeBlue) {
        ...
        if (activeBlue.isGk) {
            ...
            // Prevent entering Red box smoothly from ALL sides if Red GK has ball
            if (redGkHasBall) {
        ...
        if (ball.x - ball.radius <= 25) {
            if (ball.y >= 200 && ball.y <= 400) {
                ...
                if (ball.x - ball.radius <= 5) {
                    score.blue++;
                    triggerGoal('BLUE TEAM SCORES!', 'red');
                    return;
                }
            } else {
        ...
        if (ball.x + ball.radius >= 875) {
            if (ball.y >= 200 && ball.y <= 400) {
                ...
                if (ball.x + ball.radius >= 895) {
                    score.red++;
                    triggerGoal('RED TEAM SCORES!', 'blue');
                    return;
                }
            } else {
...
function draw() {
    ...
    if (currentState === 'INSTRUCTIONS') {
        ...
        ctx.fillText('• Goalkeepers are protected: Opponents cannot enter box or tackle GK.', 450, 360);
